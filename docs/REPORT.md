...
Also the task vagueness mattered a lot. Keeping the exact same injected page, I changed only the instruction — a *pinned* task that names the recipient ("send it to the client delivery contact in the project brief") versus the *delegated* task above. GPT-5.5 went from **0 / 21 misled** (pinned) to **22 / 22** (delegated). In other words, the injected note only does damage when the agent has to decide the recipient on its own. If the user already names the recipient, the agent never needs to read the note, so it is ignored. The attack needs both pieces together: the malicious content on the page, **and** a task vague enough that the agent has to fill in the recipient from the page.

To push the story a bit further with our delegated version, I varied only the archive domain to check whether GPT was really being injected or just following a plausible-looking process. With the realistic typosquat (`robospark-studio-assets.com`) it was misled 100% of the time, and even with an completely unrelated domain (`brand-archive-svc.net`) it still went 15/15. Only when I made the domain obviously fake (`randomsussite.net`) did it start catching it  and even then it leaked about half the time (7/15). So GPT applies at best a weak surface-level "does this look like a real service" check, with no real check against the client's actual domain. I didn't run Opus on these variations since it already refused the base case, so they weren't necessary.
